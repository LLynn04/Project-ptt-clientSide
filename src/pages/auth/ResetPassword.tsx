...
const ResetPasswordForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.otp) {
      setError("OTP is required. Please verify your OTP first.")
      return
    }
    ...
    try {
      console.log("[v0] Sending reset password request with data:", {
      ...
      console.log("[v0] Reset password response:", data)

      if (!response.ok) {
        if (data.data) {
          console.log("[v0] Validation errors:", data.data)
          const errorMessages = Object.values(data.data).flat().join(", ")
          throw new Error(errorMessages || data.message || "Password reset failed")
        }
